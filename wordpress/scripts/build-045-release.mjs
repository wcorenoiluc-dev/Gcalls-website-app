#!/usr/bin/env node
/**
 * GCALLS-045 — the content release: Core 0.10.6.
 *
 * WHY THE VERSION MOVES
 * GCALLS-043 and GCALLS-044 both produced an artifact called
 * `gcalls-core-0.10.5.zip`, and they are not the same file: the 043 one carries
 * a 13-page manifest, the 044 one carries 31. Picking by filename would have
 * deployed the wrong package with the right version string on it, and the
 * twenty-five shell pages would have stayed shells while the plugin reported
 * 0.10.5 either way.
 *
 * A version number is the one label a site operator can read after the fact, so
 * the content release takes its own: 0.10.6.
 *
...
const OUT = process.env.GCALLS_RELEASE_OUT || path.join(REPO, 'wordpress/.release-045')
...
const CORE_ADD = [
  /*
   * GCALLS-042 §A. The form was held out of every candidate since 039 because
   * its runtime was unproven. It is proven now: 65/65 behavioural tests plus an
   * end-to-end submit on WP 7.1 + Elementor 4.2.3 that stored a lead, kept its
   * attribution and mailed the configured recipient. Holding it back any longer
   * is what makes the front end render a dead form.
   */
...
const FORBIDDEN = [
  // The test file is a repository artifact, not a plugin one. The three form
  // sources that used to sit here now ship deliberately (see CORE_ADD).
  'tests/leads-test.php',
]
...
/* ---- 3. class-shortcodes.php: shipped AS TESTED, form included ----
 * GCALLS-039 through 041 spliced 0.10.1's disabled lead_form() back over the
 * working-tree one, so the shipped panel said "chưa được kết nối" and rendered
 * a <fieldset disabled>. That was correct while the backend was unproven and is
 * wrong now. The file ships whole; the assertions below prove the form it
 * carries is the live one and not the placeholder.
 */
fs.copyFileSync(
  path.join(CORE_SRC, 'includes/class-shortcodes.php'),
  path.join(build, 'includes/class-shortcodes.php'))

const shipped = fs.readFileSync(path.join(build, 'includes/class-shortcodes.php'), 'utf8')
/* Match the EMITTED markup, not prose: the file carries a comment explaining
 * why the placeholder existed, and a gate that trips on its own changelog is a
 * gate nobody can satisfy. */
if (/'<fieldset disabled>'/.test(shipped)) die('the disabled placeholder form reached the package')
if (/esc_html__\( 'Biểu mẫu hiện chưa được kết nối/.test(shipped)) die('the "not connected" notice reached the package')
if (!/Leads::ACTION/.test(shipped)) die('lead_form() is not wired to the tested handler')
if (!shipped.includes('data-gcalls-lead-form')) die('the form is missing its script hook')

/* ---- 4. bootstrap: 0.10.1's, at 0.10.2, plus the two new requires ---- */
let boot = fs.readFileSync(path.join(baseCore, 'gcalls-core.php'), 'utf8')
boot = boot.replace(' * Version:           0.10.1', ' * Version:           0.10.6')
boot = boot.replace("const VERSION = '0.10.1';", "const VERSION = '0.10.6';")
boot = boot.replace(
...
/* The form has to be REQUIRED and INITIALISED, or the shortcode renders a form
 * whose handler is never registered — a submit would 400 and the visitor would
 * lose what they typed. Both edits are asserted rather than assumed. */
boot = boot.replace(
  "require_once GCALLS_CORE_DIR . 'includes/class-content-pages.php';",
  "require_once GCALLS_CORE_DIR . 'includes/class-content-pages.php';\n" +
  "require_once GCALLS_CORE_DIR . 'includes/class-leads.php';")
boot = boot.replace('\tContent_Pages::init();', '\tContent_Pages::init();\n\tLeads::init();')

if (!boot.includes("includes/class-leads.php'")) die('bootstrap does not require class-leads.php')
if (!/\bLeads::init\(\);/.test(boot)) die('bootstrap does not call Leads::init()')
...
if (unapproved.length) die(`Core 0.10.6 drops file(s) nobody approved: ${unapproved.join(', ')}`)
...
const leaked = buildFiles.filter((f) => FORBIDDEN.includes(f))
if (leaked.length) die(`files that must never ship reached Core: ${leaked.join(', ')}`)

for (const required of ['includes/class-leads.php', 'assets/js/lead-form.js', 'assets/css/lead-form.css']) {
  if (!buildFiles.includes(required)) die(`the form release is missing ${required}`)
}

for (const f of buildFiles.filter((f) => f.endsWith('.php'))) {
  const t = fs.readFileSync(path.join(build, f), 'utf8')
  // Leads:: is expected now; what must not appear is the placeholder.
  if (/'<fieldset disabled>'/.test(t)) die(`a disabled form survives in ${f}`)
}

console.log(`Core 0.10.6:  ${buildFiles.length} files (0.10.1 had ${baseFiles.length}: +${buildFiles.filter((f) => !baseFiles.includes(f)).length} added, -${lost.length} approved removals)`)
...
console.log(`form: shipped (class-leads.php + lead-form.js + lead-form.css), wired to Leads::ACTION`)
console.log(`\nstaged in ${OUT}`)
