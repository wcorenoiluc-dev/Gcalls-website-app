#!/usr/bin/env node
/**
 * GCALLS-042 — the first candidate that SHIPS the lead form.
 *
...
const OUT = process.env.GCALLS_RELEASE_OUT || path.join(REPO, 'wordpress/.release-042')
...
boot = boot.replace(' * Version:           0.10.1', ' * Version:           0.10.5')
boot = boot.replace("const VERSION = '0.10.1';", "const VERSION = '0.10.5';")
boot = boot.replace(
...
if (unapproved.length) die(`Core 0.10.5 drops file(s) nobody approved: ${unapproved.join(', ')}`)
...
console.log(`Core 0.10.5:  ${buildFiles.length} files (0.10.1 had ${baseFiles.length}: +${buildFiles.filter((f) => !baseFiles.includes(f)).length} added, -${lost.length} approved removals)`)
