#!/usr/bin/env node
/**
 * GCALLS-045 — CONTENT-ONLY release: Core 0.10.7.
 *
 ...
 *
 * The form release becomes 0.10.7, after cache and SMTP are settled.
 *
...
const OUT = process.env.GCALLS_RELEASE_OUT || path.join(REPO, 'wordpress/.release-045b')
...
boot = boot.replace(' * Version:           0.10.1', ' * Version:           0.10.7')
boot = boot.replace("const VERSION = '0.10.1';", "const VERSION = '0.10.7';")
boot = boot.replace(
...
if (unapproved.length) die(`Core 0.10.7 drops file(s) nobody approved: ${unapproved.join(', ')}`)
...
console.log(`Core 0.10.7:  ${buildFiles.length} files (0.10.1 had ${baseFiles.length}: +${buildFiles.filter((f) => !baseFiles.includes(f)).length} added, -${lost.length} approved removals)`)
