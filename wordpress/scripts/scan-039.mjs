#!/usr/bin/env node
/**
 * GCALLS-039 §2 final-output scan.
 *
...
const BANNED = [
  ...
  ['legacy analytics chrome', 'Gcalls Analytics'],
  ['legacy trend markup', 'data-mock-series'],
]

/* Frames the registry approves. Anything else under product-gallery/ is a leak. */
const registry = JSON.parse(fs.readFileSync(
  process.env.GCALLS_REGISTRY || 'wordpress/.release/gcalls-core/data/media-frames.json', 'utf8'))
...
for (const file of files.sort()) {
  ...
  const hosts = [...new Set((html.match(/https?:\/\/([a-z0-9.-]+)/gi) || [])
    ...
    .filter((h) => !/^127\.0\.0\.1|^localhost|^www\.w3\.org|^gmpg\.org|^schema\.org|^(api|s)\.w\.org|^fonts\.(googleapis|gstatic)\.com/.test(h))

  perRoute.push({ route, found, hosts, expected: [...new Set(expected)] })
  hits += found.length
}
...
console.log(`\nscan-039: ${files.length} route(s), ${hits} finding(s)`)
