...
if ( ! function_exists( 'gcalls_react_shell_content_manifest_path' ) ) {
	/** @return string Absolute path of the content manifest this plugin ships (may not exist on a broken build). */
	function gcalls_react_shell_content_manifest_path() {
		return defined( 'GCALLS_REACT_SHELL_DIR' ) ? GCALLS_REACT_SHELL_DIR . 'content-manifest.json' : '';
	}
}

if ( ! function_exists( 'gcalls_react_shell_content_manifest' ) ) {
	/**
	 * The content manifest generated by the React build (see
	 * docs/content-studio/CONTRACT-0.2.0.md §1): every route in routes.json
	 * order with, for editable pages, each section's field schema and
	 * default content. Gcalls Content Studio reads this instead of declaring
	 * a schema of its own.
	 *
	 * Decoded once per request. Returns an empty array when the file is
	 * missing or invalid so a caller can `empty()`-check it safely.
	 *
	 * @return array<string, mixed>
	 */
	function gcalls_react_shell_content_manifest() {
		static $cached = null;
		if ( null !== $cached ) {
			return $cached;
		}
		$path = gcalls_react_shell_content_manifest_path();
		if ( '' === $path || ! is_readable( $path ) ) {
			$cached = array();
			return $cached;
		}
		$decoded = json_decode( (string) file_get_contents( $path ), true ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- local plugin file, not a remote resource.
		$cached  = is_array( $decoded ) && isset( $decoded['pages'] ) && is_array( $decoded['pages'] ) ? $decoded : array();
		return $cached;
	}
}
