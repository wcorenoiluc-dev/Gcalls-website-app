...
final class Content_Pages {
	...
	public static function init(): void {
		...
		/*
		 * The stylesheet ships from the PLUGIN because the plugin owns this
		 * markup — `class-content-pages.php` is the only thing that emits
		 * `gcalls-cp__*`. gc-components.css lives in the theme and is written
		 * against `.gc-page`, a vocabulary this renderer never emits, which is
		 * how these pages went live with 123 theme rules that could not match a
		 * single element. Markup and its presentation ship together now.
		 */
	...
	private static function card( array $c ): string {
		...
		if ( '' !== $href ) {
			/*
			 * THE VISIBLE TEXT AND THE ACCESSIBLE NAME ARE NOT THE SAME JOB.
			 *
			 * Sighted readers get "Tìm hiểu thêm" next to a heading that says
			 * what it leads to. Someone tabbing through, or pulling up a list
			 * of links in a screen reader, gets the link text alone — and this
			 * page can carry a dozen of them, every one reading "Tìm hiểu
			 * thêm". The aria-label carries the card's title so each link says
			 * where it goes; the visible label is unchanged.
			 */
			$label = '' !== $title
				? sprintf(
					/* translators: %s: card title. */
					__( 'Tìm hiểu thêm về %s', 'gcalls-core' ),
					$title
				)
				: __( 'Tìm hiểu thêm', 'gcalls-core' );
	...
	private static function section( array $s ): string {
		...
		if ( 'cards' === $type || 'split' === $type ) {
			...
			/*
			 * AN EMPTY ARRAY WAS NEVER THE ONLY WAY TO HAVE NOTHING TO SHOW.
			 *
			 * The guard above catches `cards: []`. It does not catch
			 * `cards: [{title:'',body:'',href:''} x5]`, which is what
			 * content-pages.json actually contains for 14 sections across 9
			 * pages — the exporter's key-vocabulary trap emptied every field
			 * but kept the array length. Each card then rendered as '', and the
			 * section shipped as a heading above a literally empty grid div:
			 * a question with no answer under it, on five live routes.
			 *
			 * So the cards are rendered FIRST and the container is only opened
			 * if something came back. If nothing did and the section has no
			 * lead either, the whole section is dropped rather than printing an
			 * orphan heading. This hides no content that exists — it declines
			 * to print a frame around content that does not.
			 */
			$cards = '';

			foreach ( $items as $c ) {
				$cards .= self::card( (array) $c );
			}

			if ( '' === trim( $cards ) ) {
				return empty( $s['lead'] ) ? '' : $open . '</section>';
			}

			$grid = 'split' === $type ? ' gcalls-cp__grid--2' : ' gcalls-cp__grid--3';
			$out  = $open . '<div class="gcalls-cp__grid' . $grid . '">' . $cards;
		...
		if ( 'steps' === $type ) {
			...
			foreach ( $steps as $st ) {
				...
				/*
				 * NO NUMBER, NO NUMBER SPAN.
				 *
				 * str_pad('', 2, '0') is "00", so a step without an `n` printed
				 * a literal "00" beside every item — visible on /bang-gia/ and
				 * on every ported step list. An absent number should render as
				 * nothing, not as zero.
				 */
				$n = trim( (string) ( $st['n'] ?? '' ) );
				...
				if ( '' !== $n ) {
					$out .= '<span class="gcalls-cp__step-n" aria-hidden="true">'
						. esc_html( str_pad( $n, 2, '0', STR_PAD_LEFT ) ) . '</span>';
				}
	 ...
	 */
	/**
	 * Loads the content-page stylesheet, and only on a page this module renders.
	 */
	public static function enqueue_assets(): void {
		...
		$path = GCALLS_CORE_DIR . 'assets/css/content-pages.css';
		...
		/*
		 * Depends on the theme handle so it loads after theme.css and wins on
		 * order rather than on `!important` — but only if that handle exists. A
		 * dependency WordPress cannot resolve silently drops the style
		 * entirely, and "the page is unstyled" is the bug being fixed here.
		 */
		$deps = wp_style_is( 'gcalls-theme', 'registered' ) ? array( 'gcalls-theme' ) : array();

		wp_enqueue_style(
			'gcalls-content-pages',
			GCALLS_CORE_URL . 'assets/css/content-pages.css',
			$deps,
	...
	/**
	 * Whether this request will be rendered into, decided before the loop runs.
	 *
	 * `target()` minus the two conditions that cannot hold this early:
	 * `in_the_loop()` is false at `wp_enqueue_scripts`, and there is no
	 * $content argument, so the shell test runs against raw post_content. That
	 * is the same string render() sees — `the_content` at priority 9 runs
	 * before wptexturize, and nothing earlier changes whether a body is empty.
	 *
	 * @return bool
	 */
	private static function will_render(): bool {
		if ( is_admin() || wp_doing_ajax() || is_feed() || is_preview() || is_embed() ) {
			return false;
		}
		...
		// The Elementor front page is never a target, so it never loads this CSS.
		if ( (int) get_option( 'page_on_front' ) === (int) $post->ID ) {
			return false;
		}
		...
		return self::is_empty_shell( (string) $post->post_content );
	}

	/**
	 * Whether this post will have a lead form rendered into it.
	 *
	 * The form assets are enqueued on `wp_enqueue_scripts`, which runs long
	 * before `the_content`. Leads::assets() answers the question by looking for
	 * the shortcode in post_content and _elementor_data — correct for the home
	 * page, and wrong here, because a content page's body is empty and its form
	 * comes from the manifest. Without this the contact page rendered a form
	 * with no stylesheet: unstyled, full width, and with the honeypot visible.
	 *
	 * @param \WP_Post $post Post being rendered.
	 * @return bool
	 */
	public static function renders_form( \WP_Post $post ): bool {
		if ( 'page' !== $post->post_type ) {
			return false;
		}

		$page = self::manifest()[ $post->post_name ] ?? null;
		...
		return array() !== $slot && 'blocked_runtime' !== (string) ( $slot['state'] ?? '' );
	}
	...
	private static function form_slot( array $page ): string {
		...
		/*
		 * GCALLS-042 §A/§C. `state` is the flag that kept this page showing a
		 * note instead of a form. It stays a flag rather than becoming an
		 * unconditional render, because the note is still the right output on a
		 * build that does not carry the backend — and that is now something
		 * this method can check rather than assume.
		 *
		 * ONE COMPONENT: the page calls the same shortcode the home page calls,
		 * so there is one implementation of every state, guard and field.
		 */
		$state     = (string) ( $slot['state'] ?? '' );
		$available = shortcode_exists( 'gcalls_lead_form' ) && class_exists( 'Gcalls\\Core\\Leads' );

		if ( 'blocked_runtime' !== $state && $available ) {
			$attribution = (array) ( $page['attribution'] ?? array() );
			$atts        = '';

			// The page's own attribution reaches the form as shortcode
			// attributes, so a lead from here is labelled source=contact
			// without a query string and without a second mechanism.
			foreach ( array( 'intent', 'source', 'product', 'solution' ) as $key ) {
			...
			return '<section class="gcalls-cp__section">'
				. do_shortcode( '[gcalls_lead_form' . $atts . ']' )
				. '</section>';
		}
