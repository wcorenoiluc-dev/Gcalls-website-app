...
final class Mockups {
	...
	/**
	 * A row of figure tiles — totals, percentages, averages.
	 *
	 * THE LABEL IS PART OF THE COMPONENT, NOT OF THE CALLER.
	 *
	 * Every caller of this helper is by definition printing an aggregate
	 * number, and an aggregate number on a product page reads as a measured
	 * result unless something says otherwise. The bottom caption was doing that
	 * job at the far end of a tall panel, which on mobile can be a screen and a
	 * half away from the tiles themselves. Putting the label here means a
	 * future caller cannot add a KPI strip and forget it.
	 *
	 * @param array<int, array{0: string, 1: string}> $items label/value pairs.
	 * @return string
	 */
	private static function tiles( array $items ): string {
		$out  = '<div class="gcalls-tiles-wrap">';
		$out .= '<p class="gcalls-tiles__label">' . esc_html__( 'Dữ liệu minh hoạ', 'gcalls-core' ) . '</p>';
